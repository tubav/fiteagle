/*
 * $Id: DiameterWorker.java 1950 2010-07-16 13:07:50Z dvi $
 *
...
public class DiameterWorker extends Thread {
	...
	/** The logger */
	private static final Logger LOGGER = Logger.getLogger(DiameterWorker.class.toString());
...
//			if (listeners.size()>0) ((EventListener) listeners.get(0)).recvMessage(task.peer.FQDN,task.msg);
			...
			for(i=0;i<size;i++){
				e = listeners.get(i);
				LOGGER.fine(task.msg.toString());
				if (task.msg.flagRequest)
					e.recvRequest(task.peer.FQDN,(Request)task.msg);
				else
					e.recvAnswer(task.peer.FQDN,(Answer)task.msg);
			}
		}
