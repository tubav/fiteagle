/*
 * $Id: AVP_QoSFilterRule.java 1979 2010-07-20 13:08:28Z dvi $
 *
...
import de.fhg.fokus.diameter.DiameterPeer.data.avp.derived.filterrule.QoSFilterRule;
import de.fhg.fokus.diameter.DiameterPeer.data.codec.AVPDecodeException;



public class AVP_QoSFilterRule extends AVP {

	private QoSFilterRule qosFilterRule;
	private boolean dirtyAVPData=false;

	public AVP_QoSFilterRule(AVP_Type type,byte[] data) throws AVPDecodeException
	{
		super(type,data);		
		this.qosFilterRule = new QoSFilterRule(new String(data));
	}

	public AVP_QoSFilterRule(int code, boolean mandatory, long vendorId,String data) throws AVPDecodeException
	{
		super(code,mandatory,vendorId);
		this.set(data);
	}

	public AVP_QoSFilterRule(AVP_Type type, String data) throws AVPDecodeException {
		super(type);
		this.set(data);
	}
	...
	public synchronized void setRaw(byte[] data,int start,int len) throws AVPDecodeException
	{
		...
		this.qosFilterRule = new QoSFilterRule(new String(this.avpData));							
	}
	
	public synchronized byte[] getRaw() {
		if (dirtyAVPData) {
			if (qosFilterRule!=null)
				this.avpData = qosFilterRule.toString().getBytes();
			else
	...
	{
		this.qosFilterRule = new QoSFilterRule(data);
		this.dirtyAVPData = true; 
	}
	public synchronized void set(QoSFilterRule data)
	{
		this.qosFilterRule = data;
		this.dirtyAVPData = true; 
	}
	
	public synchronized QoSFilterRule get()
	{
		return this.qosFilterRule;
	}
	...
	public synchronized String toString()
	{
		...
		x.append(" QoSFilterRule_Data=");x.append(this.qosFilterRule.toString());
		return x.toString();
	}
