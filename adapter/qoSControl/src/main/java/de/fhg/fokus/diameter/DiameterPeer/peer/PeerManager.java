/*
 * $Id: PeerManager.java 1980 2010-07-20 14:37:22Z dvi $
 *
...
public class PeerManager extends Thread {
	...
	public Peer configurePeer(String fqdn,String realm,int port)
	{
		...
		LOGGER.fine("PeerManager: Peer "+fqdn+":"+port+" added.");
		peers.add(p);
		return p;
	}
...
//		}
	...
	/**
	 * Add Peers detected dynamically to the Peer list, if it is allowed.
	 *
	 * @param fqdn	Fully Qualified Domain Name of a Peer.
	 * @param realm Realm name of a Peer.
	...
	public void run() 
	{
		...
		while (running){
			...
			//LOGGER.warning("-Peer list-");
			for(i=0;i<peers.size();i++){
				p = (Peer) peers.get(i);
				//LOGGER.warning("Peer State:"+p.state+" FQDN:"+p.FQDN+":"+p.port);
			}
			//LOGGER.warning("-----------");
			for(i=0;i<peers.size();i++){
				p = (Peer) peers.get(i);
				if (p.lastReceiveTime<expiration){
					switch (p.state){
						...
						case Closed:
							...
							LOGGER.fine("Connecting to peer "+p.FQDN+":"+p.port+" dynamic "+p.isDynamicPeer+" dropping "+ diameterPeer.DropUnknownOnDisconnect);
							p.refreshTimer();
							PeerStateMachine.process(p,PeerEvent.Start);
							break;
						/* timeouts */	
						case Wait_Conn_Ack:
						case Wait_I_CEA:
						...
						case Wait_Returns:
							p.refreshTimer();
							PeerStateMachine.process(p,PeerEvent.Timeout,null);
							break;	
						/* inactivity detected */
						case I_Open:
						case R_Open:
							if (p.waitingDWA){
								p.waitingDWA = false;
								if (p.state==PeerState.I_Open) PeerStateMachine.process(p,PeerEvent.I_Peer_Disc);
								if (p.state==PeerState.R_Open) PeerStateMachine.process(p,PeerEvent.R_Peer_Disc);
							} else {
								p.waitingDWA = true;
								if (!PeerStateMachine.Snd_DWR(p)){
									if (p.state==PeerState.I_Open) PeerStateMachine.process(p,PeerEvent.I_Peer_Disc);
									if (p.state==PeerState.R_Open) PeerStateMachine.process(p,PeerEvent.R_Peer_Disc);									
								} else p.refreshTimer();
							}
...
//			if (p.R_comm!=null) p.R_comm.shutdown();
			PeerStateMachine.process(p,PeerEvent.Stop);
		}
	}
}
