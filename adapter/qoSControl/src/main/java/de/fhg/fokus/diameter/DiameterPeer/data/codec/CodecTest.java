/*
 * $Id: CodecTest.java 1945 2010-07-15 14:52:42Z dvi $
 *
...
/**
 * This class tests the DiameterCodec for correct behavior
 * 
...
public class CodecTest {
	...
	/** The logger */
	private static final Logger LOGGER = Logger.getLogger(CodecTest.class.getName());
	...
	private static void speedTest(String[] args) {
		...
		int repeat = 1000000;
		if (args.length<1) {
			LOGGER.log(Level.FINE, "No input file specified. Please specify it as first parameter!");
			System.exit(-1);
		}
		if (args.length<2) {
			LOGGER.log(Level.FINE,"No output file specified. Please specify it as second parameter!");
			System.exit(-1);
		}
		original = readData(args[0]);
		if (original==null){
			LOGGER.log(Level.FINE,"Error reading data from file. Aborting!");
			System.exit(-1);			
		}
		duration = System.currentTimeMillis();
		try {
			for(int i=0;i<repeat;i++)
				msg = Codec.decodeMessage(original,0);
		} catch (MessageDecodeException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			LOGGER.log(Level.FINE,"Error decoding AVP. Aborting!");
			System.exit(-1);			
		}
//		try {
//			for(int i=0;i<repeat;i++)
...
//			// TODO Auto-generated catch block
//			e1.printStackTrace();
//			LOGGER.log(Level.FINE,"Error Un-grouping AVP. Aborting!");
//			System.exit(-1);			
//		}
		for(int i=0;i<repeat;i++)
			bootstrapped = Codec.encodeMessage(msg);

		duration = System.currentTimeMillis() - duration;
		
		LOGGER.log(Level.FINE,"Speed: "+repeat/duration*1000+" ops/sec");
		LOGGER.log(Level.FINE,msg.toString());
				
		bootstrapped = Codec.encodeMessage(msg);
		if (!writeData(args[1],bootstrapped)){
			LOGGER.log(Level.FINE,"Error writing data to file. Aborting!");
			System.exit(-1);					
		}
		if (Arrays.equals(original,bootstrapped)){
			LOGGER.log(Level.FINE,"Bootstrapping Succesfull...");		
		}
		else{
			LOGGER.log(Level.FINE,"Bootstrapping failed!");
			System.exit(-1);								
		}
...
//		}
	...
	private static void codecTest() {
		AVP x=null;
		try {
			x = new AVP_Result_Code(2003);
		} catch (AVPDataTypeException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		LOGGER.log(Level.INFO,x.toString());
		LOGGER.log(Level.FINE,AVP_Result_Code_Enum.DIAMETER_AUTHENTICATION_REJECTED.avp.toString());
		
			x = new AVP_Grouped(AVP_Type.Experimental_Result,new AVP[] {x, AVP_Disconnect_Cause_Enum.BUSY.avp});
		LOGGER.log(Level.INFO,x.toString());
		
		
		CER cer = new CER();
		LOGGER.log(Level.INFO,cer.toString());
	}
