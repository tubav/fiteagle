/*
 * $Id: Acceptor.java 1948 2010-07-15 17:01:34Z dvi $
 *
...
public class Acceptor extends Thread {
	...
	/** The logger */
	private static final Logger LOGGER = Logger.getLogger(Acceptor.class.toString());
	...
	private void openSocket()
	{
		try {
			...
			LOGGER.severe("Acceptor: Error opening socket on port "+port);
			e.printStackTrace();
		}
		try {
			acceptSocket.setSoTimeout(so_timeout);
		} catch (SocketException e1) {
			LOGGER.severe("Acceptor: Error setting SO_TIMEOUT socket of "+so_timeout);
			e1.printStackTrace();
		}
	}
	...
	private void closeSocket()
	{
		try {
			...
			LOGGER.severe("Acceptor: Error closing socket on port "+port);
			e.printStackTrace();
		}
	}	
	 ...
	 * be associated with a proper Peer in the PeerManager.
	 * 	 
	...
	public void run() {
		...
		while(accepting){
			try {
				...
				LOGGER.severe("Acceptor: SO_TIMEOUT -> No Connection");
				continue;
			} catch (IOException e) {
				if (accepting){
					LOGGER.severe("Acceptor: I/O Error on accept");
					e.printStackTrace();
				}
				break;
			} 
			LOGGER.fine("Acceptor: Connection Accepted");
